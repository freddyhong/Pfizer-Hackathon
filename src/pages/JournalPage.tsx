...
function generateRandomData(scale: number, days = 75): DataPoint[] {
...
type MoodGraphProps = { data: DataPoint[] };
const MoodGraph = ({ data }: MoodGraphProps) => (
  <div className="mb-8">
    <div className="flex justify-between items-center mb-2">
      <h2 className="text-2xl font-bold tracking-wide text-[#488CCA]">
        Mood Log
      </h2>
    </div>
    <div
    ...
    >
      <ResponsiveContainer width="100%" height={220}>
        ...
        >
          ...
          <Tooltip formatter={(val) => emojisMood[Number(val)]} />
          <Line
            type="monotone"
            dataKey="value"
            stroke={PFIZER.blue5}
...
type PainGraphProps = { data: DataPoint[] };
const PainGraph = ({ data }: PainGraphProps) => (
  <div className="mb-8">
    <div className="flex justify-between items-center mb-2">
      <h2 className="text-2xl font-bold tracking-wide text-[#272C77]">
        Pain Log
      </h2>
    </div>
    <div
    ...
    >
      <ResponsiveContainer width="100%" height={220}>
        ...
        >
          ...
          <CartesianGrid strokeDasharray="3 3" />
          <Tooltip formatter={(val) => emojisPain[Number(val)]} />
          <Line
            type="monotone"
            dataKey="value"
            stroke={PFIZER.blue3}
...
const FeelingCard = ({ date, words, index }: FeelingCardProps) => (
  <div
...
export default function DailyJournal() {
  const [fullMoodData, setFullMoodData] = useState<DataPoint[]>([]);
  const [fullPainData, setFullPainData] = useState<DataPoint[]>([]);
  ...
  const monthlyMoodData: DataPoint[] = useMemo(() => {
  ...
  const monthlyPainData: DataPoint[] = useMemo(() => {
    const year = currentMonth.getFullYear();
    const month = String(currentMonth.getMonth() + 1).padStart(2, "0");
    const prefix = `${year}-${month}`;
  ...
  const feelingWords = useMemo<{ date: string; words: string[] }[]>(() => {
  ...
  const avgMood =
    monthlyMoodData.length > 0
      ? (
          monthlyMoodData.reduce((s, d) => s + d.value, 0) /
          monthlyMoodData.length
        ).toFixed(1)
      : "-";
  const avgPain =
    monthlyPainData.length > 0
      ? (
          monthlyPainData.reduce((s, d) => s + d.value, 0) /
          monthlyPainData.length
        ).toFixed(1)
      : "-";
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(270deg, ${PFIZER.blue5}, ${PFIZER.blue8})`,
      }}
    >
      <header
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <h2 className="text-xl font-bold text-[#488CCA]">
            Overview of Month
          </h2>
          ...
          <div className="bg-white/80 rounded-xl border border-blue-200 px-4 py-3 text-sm">
            <p>
              Zach has been suffering through lots of pain this week. He's been
              mostly unhappy for the past 3 days.
            </p>
          </div>
          ...
          <div className="bg-white/80 rounded-xl border border-blue-200 px-4 py-3 text-sm">
            <ul className="list-disc list-inside">
              ...
              <li>
                Encourage positive reinforcement through character rewards.
              </li>
